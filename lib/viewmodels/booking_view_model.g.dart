// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'booking_view_model.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic, no_leading_underscores_for_local_identifiers

mixin _$BookingViewModel on _BookingViewModelBase, Store {
  late final _$responseBodyAtom =
      Atom(name: '_BookingViewModelBase.responseBody');

  @override
  List<dynamic> get lessons {
    _$responseBodyAtom.reportRead();
    return super.lessons;
  }

  @override
  set responseBody(List<dynamic> value) {
    _$responseBodyAtom.reportWrite(value, super.lessons, () {
      super.lessons = value;
    });
  }

  @override
  ObservableFuture<List<LessonResponseModel>?> LessonsByBranchName(
      String? Location) {
    final _$future = super.LessonsByBranchName(Location);
    return ObservableFuture<List<LessonResponseModel>?>(_$future);
  }

  @override
  String toString() {
    return '''
responseBody: ${lessons}
    ''';
  }
}
